...
class Agent:
    ...
    def build_index(self) -> tuple[list[str], list[str], faiss.IndexFlatIP]:
        ...
        if index is None:
            raise ValueError(f"Empty knowledge base: {kb_path}")
    ...
    async def search(self, query: str) -> list[tuple[str, str, float]]:
        ...
        scores, indices = self.index.search(query_vec, self.config.retrieval.top_k)
        return [
...
def main() -> None:
    config = Config.model_validate(
        yaml.safe_load(APP_DIR.joinpath("config.yaml").read_text()),
    )
    agent = Agent(config)
